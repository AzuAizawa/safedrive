// Cancellation and no-show terms (CHAPTER 91), modelled on Turo's policy.
//
//   free        cancelled at least refund_full_hours before pickup - or, for a
//               booking paid closer to pickup than that, within
//               short_notice_free_hours of the first payment, never past pickup
//   late cancel late_cancel_fee_days of the booking's average daily cost, or
//               short_trip_late_cancel_fee_days for a trip of
//               SHORT_TRIP_MAX_DAYS or fewer
//   no-show     the renter never came, or cancelled after the pickup time:
//               no_show_fee_days / short_trip_no_show_fee_days
//
// The fee is counted on what the booking costs per day, never on what has been
// paid so far, so a renter who paid in full and one who paid the downpayment
// are charged the same. It is only capped at what was paid.
//
// A booking made before CHAPTER 91 has no fee-day snapshots and keeps the
// percentage terms it was sold under (refund_late_renter_percent_snapshot).
//
// Pure and import-free. This copy decides the refund; src/lib/cancellationPolicy.ts
// is the same code for what renters and listers are shown, and
// scripts/cancellation-policy.test.mjs pins the two to the same answers.
