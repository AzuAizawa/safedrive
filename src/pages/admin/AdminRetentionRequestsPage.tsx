...
export default function AdminRetentionRequestsPage() {
  ...
  const advance = async (item: Request) => {
    if (!user?.id) return;
    const next = item.status === "submitted" ? "identity_check" : item.status === "identity_check" ? "under_review" : null;
    if (!next) return;
  ...
  const markExecuted = async (item: Request) => {
    if (!user?.id) return;
    const proof = window.prompt("Describe exactly what was exported, corrected, deleted, anonymized, or restricted. Do not include secrets.");
    if (!proof?.trim() || proof.trim().length < 10) return;
    const completedAt = new Date().toISOString();
    const { error } = await supabase.from("data_retention_requests").update({ status: "executed", completed_at: completedAt, decision_reason: `${item.decision_reason || "Approved."} Execution: ${proof.trim()}` }).eq("id", item.id).eq("status", "approved");
    if (error) toast.error("Execution was not recorded", { description: error.message });
    else { await supabase.from("audit_log").insert({ user_id: user.id, action: "data_request_executed", entity_type: "data_retention_request", entity_id: item.id, details: { completed_at: completedAt, execution_proof: proof.trim() } }); await load(); }
  };
