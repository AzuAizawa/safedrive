...
export default function AdminReconciliationPage() {
  ...
  // One page at a time with the total. It used to be the newest 500, so an
  // older open issue could drop off the list for good.
  const load = useCallback(async () => { setLoading(true); const { from, to } = pageRange(page); const { data, error, count } = await supabase.from("reconciliation_items").select("*", { count: "exact" }).order("created_at", { ascending: false }).range(from, to); if (error?.code === "PGRST103" && page > 1) { setPage(1); return; } if (error) toast.error("Reconciliation items could not be loaded", { description: error.message }); else { setItems(data ?? []); setTotal(count ?? 0); } setLoading(false); }, [page]);
