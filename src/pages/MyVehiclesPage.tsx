    ...
    <label className="relative flex h-24 w-full cursor-pointer flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed px-2 text-center transition-colors hover:border-primary/50 aria-[current=true]:border-green-500/50 aria-[current=true]:bg-green-500/5" aria-current={Boolean(file)}>
      {file && (
        <button
          ...
          onClick={(event) => {
            // Inside a label, so the click has to be stopped or it reopens
            // the file dialog it was meant to cancel.
      ...
      <input
        ...
        onChange={(event) => {
          ...
          if (validateUploadFile(chosen, allowedTypes, validationLabel)) {
            onFile(chosen);
          } else {
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <DocumentField
                  ...
                  validationLabel="OR document"
                  accept="image/jpeg,image/png,image/webp"
                  expiryLabel="Registration expiry shown on the OR *"
                  expiryValue={form.registration_expiry}
                  onExpiry={(value) => setForm({ ...form, registration_expiry: value })}
                  expiryRequired
                />
                <DocumentField
                  ...
                  onFile={setCrFile}
                  allowedTypes={ALLOWED_IMAGE_TYPES}
                  validationLabel="CR document"
                  accept="image/jpeg,image/png,image/webp"
                  note="The CR does not expire, so no date is needed."
                />
                <DocumentField
                  ...
                  onFile={setCtplFile}
                  allowedTypes={ALLOWED_IMAGE_TYPES}
                  validationLabel="CTPL document"
                  accept="image/jpeg,image/png,image/webp"
                  expiryLabel="Expiry date shown on the CTPL *"
                  expiryValue={form.ctpl_expiry}
                  onExpiry={(value) => setForm({ ...form, ctpl_expiry: value })}
                  expiryRequired
                />
                <DocumentField
                ...
                <DocumentField
                  ...
                  onFile={setDtiFile}
                  allowedTypes={ALLOWED_BUSINESS_DOCUMENT_TYPES}
                  validationLabel="DTI document"
                  accept="image/jpeg,image/png,image/webp,application/pdf"
                  expiryLabel="Expiry date shown on the DTI registration *"
                  expiryValue={form.dti_expiry}
                  onExpiry={(value) => setForm({ ...form, dti_expiry: value })}
                  expiryRequired
                />
                <DocumentField
                  ...
                  allowedTypes={ALLOWED_BUSINESS_DOCUMENT_TYPES}
                  validationLabel="Permit document"
                  accept="image/jpeg,image/png,image/webp,application/pdf"
                  expiryLabel="Expiry date shown on the permit *"
                  expiryValue={form.mayors_permit_expiry}
                  onExpiry={(value) => setForm({ ...form, mayors_permit_expiry: value })}
                  expiryRequired
                />
                <DocumentField
                  ...
                  onFile={setBirFile}
                  allowedTypes={ALLOWED_BUSINESS_DOCUMENT_TYPES}
                  validationLabel="BIR document"
                  accept="image/jpeg,image/png,image/webp,application/pdf"
                  note="Form 2303 does not expire, so no date is needed."
                />
              </div>
