...
export default function TripConditionReportPage() {
  ...
  const anyFile = useMemo(() => allPhotoSlots.some(([category]) => files[category]), [files]);
        ...
        <p className="mt-2 text-sm">
          {photosRequired ? (
            <span className="font-medium text-foreground">
              This report is required for you {validPhase === "pickup" ? "as the lister (the “before” state)" : "as the renter (the “after” state)"} - the 4 photos below are required.
            </span>
          ) : (
