...
A lister may optionally set `cars.min_early_return_notice_hours` (CHAPTER 38, 0-72) - shown to the renter as a hint before they send a request. It is purely informational: the API does not read or enforce it, and the lister can still approve or decline any request regardless of the notice actually given.
