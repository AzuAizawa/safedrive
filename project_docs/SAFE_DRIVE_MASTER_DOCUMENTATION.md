...
| `api/booking-early-return-action.ts` | POST; participant | Request (date+time) / approve / reject / cancel an early return; approve never touches `bookings.end_date`/`dropoff_time` (§8.4's missed-early-return fallback depends on this) and (optionally) opens a lister goodwill-refund review |
