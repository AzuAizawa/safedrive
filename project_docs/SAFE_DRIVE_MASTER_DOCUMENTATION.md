...
A lister sets `cars.min_early_return_notice_hours` (CHAPTER 38, tightened to a required 1-24h dropdown by CHAPTER 40) - shown to the renter as a hint before they send a request. It is purely informational: the API does not read or enforce it, and the lister can still approve or decline any request regardless of the notice actually given. The column stays nullable at the database level for cars listed before this was required.
...
- **Response deadline (CHAPTER 46):** `booking_early_returns.response_deadline` is stamped at request time - `min(now + 24h, end of the requested new return day)`, the same "never past the moment that matters" cap `payment_deadline`/`balance_deadline` already use. A pending request past its deadline is treated as a decline (`bookings.end_date` is never touched by expiry, only by an actual approval) and moves to `status = 'expired'` - a value the table's check constraint has allowed since CHAPTER 30, but nothing ever computed a deadline or set it until now. Resolved two ways: a defensive check inline in `approve`/`reject`/`cancel` closes the narrow race window before the next cron tick, and `api/expire-booking-deadlines.ts` (already running every 15 minutes) is the authoritative path, notifying both sides.
...
- `src/components/InstallPrompt.tsx` (new, mounted once at the `App.tsx` root - reaches the public landing/login/signup pages as well as the logged-in `DashboardLayout` shell, since a first-time mobile visitor should be able to install before ever creating an account; hides itself on `/admin/*` via a pathname check) - a dismissible "Install SafeDrive" banner: captures `beforeinstallprompt` on Android/Chrome, shows an "Add to Home Screen" instructional variant on iOS Safari (no install-prompt API exists there), never renders if already running standalone, 14-day dismiss cooldown in `localStorage`.
