...
- `src/components/InstallButton.tsx` - the single install entry point: a small permanent button mounted in the landing page header (`LandingPage.tsx`) and `DashboardLayout.tsx`'s header, reachable from a first-time visitor's very first screen and every logged-in page afterward, always in the same place. An earlier dismissible one-time banner (`InstallPrompt.tsx`) was tried and removed - a renter who dismissed it had no other way to install later, and a second, permanent control made the dismissible one redundant rather than complementary. **Never hides itself, for any reason** - two earlier versions did (first gated on `beforeinstallprompt` having fired, then only on `isStandalone`/already-installed) and both were reported as wrong: `beforeinstallprompt` is a one-shot-ish browser event Chrome throttles/suppresses re-firing on repeated page loads within a session, so hiding on it made the button flicker in and out across refreshes; hiding on `isStandalone` was also rejected - a user who later uninstalls the app would have no visible way back to an install control (in practice `isStandalone` is a live per-tab check that would already un-hide itself in a normal browser tab post-uninstall, but relying on that being obvious - or on any browser signal being consistent at all - is exactly the fragility that caused the original bug). A click always does something useful regardless of state: triggers the native prompt if available, explains "already installed" if `isStandalone`, shows the iOS "Add to Home Screen" steps on iOS Safari, or otherwise a generic "look for the install icon" fallback - never nothing.
