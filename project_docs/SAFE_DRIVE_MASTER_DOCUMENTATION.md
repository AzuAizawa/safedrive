...
- **`renter_no_show`** (lister, `/lister-bookings`): the lister checked in, the renter never arrived. The booking is cancelled; the renter keeps a **forfeit share (default 50%, `refund_late_renter_percent` — the same admin-configurable number the short-notice cancellation policy uses, snapshotted per booking)** and the remainder is queued as a `payments` `refund` row (`payment_method='manual_review'`) that a super-admin releases in Financial Reviews after confirming the return method. `booking_cancellations` is stamped `cancelled_by_role='renter'`, `was_late=true` — the lister's record is untouched.
...
| `api/booking-incident-action.ts` | POST; participant | Pickup / return incidents: `renter_no_car` (renter checked in, no car → cancel + full refund, no reliability hit, cascade-aware fault), `renter_no_show` (lister checked in, renter absent → cancel, renter keeps a `refund_late_renter_percent` forfeit share, default 50%), `report_non_return` (active trip overdue → `bookings.dispute_status='open'` + a structured `dispute_reason`, support case, no cancel) |
