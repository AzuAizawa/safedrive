...
- **Release the lister's compensation on the asynchronous refund path.** `api/mark-manual-refund.ts` releases it in the same click when the provider confirms immediately, which is always the case in demo mode. In live mode a refund can come back `pending`, and `api/sync-paymongo-refund.ts` - the handler that later completes it, posts the ledger journal and emails the receipt - does not call `releaseCancellationCompensation`, so the lister's share would sit unreleased with no button to reach it. The fix belongs right after the ledger post there, where the remainder becomes readable; compensation's own guards (cancelled booking only, no released payout, no other open refund, unique payout row) already prevent a double payment. This is a missing code path, not a configuration step - do not describe it as one.
...
Because that backstop never blocks, `DashboardLayout`'s chrome (the "Lister Mode"/"Renter Mode" badge, nav items, colors) prefers `portalModeForPath(location.pathname)` over the persisted flag whenever the current route implies a mode - a bookmarked or emailed link into the other mode's space shows the correct badge/nav from the first paint, instead of the stale flag for the round-trip until `ModeRoute`'s background flip resolves. The persisted flag (`profileIsLister`) is still authoritative for the toggle action itself (`handleToggleMode`'s verification gate, the DB write, and the "Switch to X" label) - only passive display reads the route first.
...
**Cross-account profile visibility (CHAPTER 43).** `public.profiles` has exactly one `for select` policy. Before this chapter it was `auth.uid() = id or is_admin()` - a non-admin account could read only its own row. Since PostgREST embeds a joined table subject to that table's own RLS, this silently broke every query anywhere in the app that embeds a *different* person's profile for a non-admin viewer: `BrowseCarsPage.tsx`'s `profiles!cars_owner_id_fkey!inner(...)` is an inner join, so the blocked embed dropped the whole car row - confirmed live, an anonymous guest's browse query returned 0 cars against a service-role ground truth of 5 - while `CarDetailPage.tsx` (a plain/left join) still showed the car but with the lister's name/phone/email/rating blank, and `MyBookingsPage.tsx`/`ListerBookingsPage.tsx` lost the other participant's info on the renter's/lister's own booking. This went unnoticed because every account used to test it was admin/super-admin, which bypasses via `is_admin()`. The policy now adds two more `or exists (...)` clauses: a profile that owns at least one `approved`/`active` car is publicly readable (including to signed-out guests, matching cars' own `USING (true)`), and a profile is readable to the other participant of any booking between them (either direction). RLS is row-level, not column-level - every affected query already requests an explicit column list, never `profiles(*)`, so this does not newly expose sensitive columns (`national_id`, `driver_license`, `address`, `birthday`, `role`, ...) through those call sites; a future `select *` on profiles would still need its own review.
...
- **Add Vehicle** (`/my-vehicles`, initial listing): registration and CTPL expiry are required; comprehensive is optional. Documents required at this stage are one OR photo and one CR photo (each a single-page document - OR and CR have no front/back) plus a CTPL document photo; a comprehensive insurance document photo is optional, matching its optional expiry date (CHAPTER 40 batch).
...
The admin's `/admin/users` licence panel has two outcomes for a pending resubmission (CHAPTER 44) - not just a single "Save": **Save licence details** accepts it (sets `license_expiry`/`license_transmission` from the form, clears `license_update_pending` and any earlier `license_rejection_reason`), or **Reject** (only shown while a resubmission is actually pending) clears `license_update_pending` **without** touching the expiry/transmission fields and requires a reason, stored in `profiles.license_rejection_reason` and shown back on the panel until the next accepted save supersedes it. Both outcomes notify the renter in-app **and** by email (`api/send-license-decision-email.ts`, the same generic account-notification template other admin decisions use) - previously only an in-app notification fired, and only on accept.
...
- **Arrival check-in** opens only from `arrival_checkin_lead_hours` before the scheduled pickup datetime (default 3 h), and the same lead time gates the return leg's "I Have Arrived" (`return_arrive`) before the scheduled drop-off datetime. `api/booking-action.ts` rejects an earlier `arrive`/`return_arrive` call; both dashboards show a "check-in opens ..." note instead of the button for both legs - reported gap: the return button used to render unconditionally the moment a trip went active, with no client-side lead-time gate mirroring the pickup one, so it could appear days before an actual drop-off with no explanation. Fixed in `MyBookingsPage.tsx`/`ListerBookingsPage.tsx` by adding `getBookingDropoffMs`/`returnCheckinOpen`, mirroring the existing pickup-side `getBookingPickupMs`/`arrivalCheckinOpen` pattern exactly.
- **Completion** is rejected before the scheduled pickup datetime - a trip that has not started cannot be finished. Early checkout is allowed any time from pickup onward.
- **Pickup arrival auto-posts to the booking chat.** Every genuine "I Have Arrived" tap (`api/booking-action.ts`'s `"arrive"` action) now also finds-or-creates the booking's `support_tickets` conversation (`tag: "booking_conversation"`, the same one pickup/return condition-report photos already post into) and appends a message ("Renter/Lister arrived at pickup.", plus a clickable Google Maps "View location" link when a location was shared) - never fails the arrival itself if the chat post fails. The lister's earlier "Confirm - Renter Is Here" override (confirming the renter's arrival on their behalf) was removed entirely, client and server, so this is now always a genuine self-arrival on both sides.
- **Sequential return handover (CHAPTER 39):** completion at return is no longer symmetric. The renter first taps "I've Returned the Car" (`renter_return_arrived_at`, action `return_arrive` - renter-only, no evidence requirement of its own). The **lister** must then inspect the car and tap "Confirm - Car Received" (`owner_completed`) - blocked until the renter's announcement is on file. Only then does the renter's own final "Car Confirm" (`renter_completed`) unlock, so the renter has their own record that the lister acknowledged the return rather than only the lister's word. A booking that reached `renter_completed=true` under the pre-CHAPTER-39 symmetric rule is grandfathered - the lister's gate also accepts a pre-existing `renter_completed` in place of `renter_return_arrived_at`.
- **Lister-absent completion:** `api/expire-booking-deadlines.ts` auto-completes the lister's side after `lister_completion_timeout_hours` (default 18) so an unreachable lister cannot hold the renter indefinitely - from either `renter_completed_at` (the pre-CHAPTER-39 path, sets `status='completed'` directly since the renter had already confirmed) or `renter_return_arrived_at` (the new sequential path, sets only `owner_completed=true` since the renter still needs to tap their own "Car Confirm" afterward).

`arrival_checkin_lead_hours` is one of the configurable lifecycle timings in `platform_settings` (`arrival_checkin_lead_hours`, `lister_completion_timeout_hours`, `no_show_grace_minutes`). Unlike the financial terms they are read **live**, not snapshotted per booking, and are changed through the same multi-super-admin consensus flow (`/admin/platform-settings`).

**A change can carry a start date (CHAPTER 69).** `platform_setting_change_requests.effective_from`: when set, an approved change sits at `status = 'scheduled'` instead of applying the moment the vote passes, and `promote_scheduled_platform_setting_changes()` applies it once the Manila date arrives - called from the existing daily `api/expire-platform-setting-changes.ts` cron, so no new endpoint or scheduler entry. Left NULL, a change applies immediately, which stays available for correcting a mistake.

Why it exists: the money terms are already frozen per booking and a live setting is read at the moment it is used, so a future booking picks up a new value on its own. The single remaining gap was a booking sitting in its pickup window at the instant an admin flipped a live timing - the renter watching "SafeDrive waits until 12:30 AM" would refresh and see 1:00 AM. A start date closes that without a snapshot column per setting, and keeps support's answer to one sentence: "from March 1 it is 60 minutes; before that, 30." At most one scheduled change at a time (`platform_setting_change_one_scheduled`), and the 7-day voting deadline is extended so it can never fall before the start date.

**Announcements (CHAPTER 69)** are the other half: `send_platform_announcement(title, message, audience)` writes one `notifications` row per recipient, so it lands in the bell users already watch (realtime-subscribed in `DashboardLayout`, so it appears without a refresh) with no new user-facing surface. Audience is `all`, `listers` (owns at least one car) or `renters` (owns none) - deliberately **not** `profiles.is_lister`, which is a per-session UI mode flag reset on every sign-out. Bell only, no email: `sendUserNotificationEmail` sends one address per Resend call, so a broadcast would time out inside one request and a free-tier daily cap would truncate it; doing email properly needs a drain-in-batches queue, deferred until the account's real Resend limit is known.
...
- **`booking_cancellations`:** one row per cancelled booking, either party. `was_late` = cancelled inside the booking's own `refund_full_hours` window — the same threshold the renter already faces, so both sides are judged symmetrically. `api/expire-booking-deadlines.ts` also writes a row (`cancelled_by_role='renter'`, `was_late=true`, `had_captured_payment=false`) when a `confirmed`/`awaiting_payment` booking auto-cancels because the renter never completed the reservation payment before `payment_deadline` — it ties up the car the same way a late cancellation does, so it counts the same way.
...
A lister sets `cars.min_early_return_notice_hours` (CHAPTER 38, tightened to a required 1-24h dropdown by CHAPTER 40), enforced against the requested instant when a renter sends a request - the lister can still approve or decline any request regardless. The column stays nullable at the database level for cars listed before this was required.

- **Request** (renter, `/my-bookings`): booking `fully_paid` / `active`, not yet being completed, no open extension; the new **date and time** must be earlier than the current agreed return instant, the date after pickup, and not in the past. A specific time is required (CHAPTER 56), not just a date - a same-day request (e.g. original drop-off 10 AM, renter wants 6 AM the same day) is valid as long as the combined instant is earlier.
- **Approve** (lister, `/lister-bookings`): **deliberately never touches `bookings.end_date`/`dropoff_time`** (CHAPTER 56) - those columns permanently mean "the original agreed return date+time" and stay the sole input to every availability/overlap check (the `bookings_no_active_date_overlap` exclusion constraint, `create-booking.ts`'s overlap check, `booking-extension-action.ts`'s day-math anchor, both calendars), so the "freed" days are never actually released while the booking is non-terminal. The approved early date+time lives only on the `booking_early_returns` row. Approving moves only the return time: once the vehicle is handed over, nothing paid is refunded - not for an early return, unused days or paid extension days (CHAPTER 98). The optional lister "goodwill refund" this once offered was removed (it was off-policy, and SafeDrive would have paid it because the lister's payout was never reduced); `booking_early_returns_no_goodwill_refund` holds the column at zero. The one refund after handover is an extension payment that could not be applied (`api/webhooks/paymongo.ts`), refunded through support review.
- **Missed-early-return fallback (CHAPTER 56):** if neither party confirms return arrival by the approved early instant plus its 30-minute no-show grace, the return flow automatically falls back to treating the *original* agreed instant as operative again - a missed early checkout is not a harder deadline than the one the trip started with. This is stateless (recomputed live from arrival timestamps, not a persisted flag): `getOperativeReturnDeadline` (`src/lib/bookingLifecycle.ts`) drives the reminder/overdue banner and no-show-report eligibility and can fall back this way; `getReturnCheckinEligibleDeadline` drives only the "I Have Arrived" button's own lead-time gate and, once an early return is approved, never re-closes even after the fallback makes the original instant operative again for labeling purposes - otherwise the button would flicker shut right as the missed party needs it most. The reminder banner's "overdue" label itself only appears `RETURN_OVERDUE_LABEL_GRACE_MINUTES` (180, independent of the 30-minute no-show grace) after whichever deadline is operative.
- **Reject** keeps the original end date and full amount. Both sides are notified (in-app + email); the Platform Agreement §4 ("Early Return and No Refund After Handover") states the no-refund rule.
- **Response deadline (CHAPTER 46):** `booking_early_returns.response_deadline` is stamped at request time - `min(now + 24h, the requested instant itself)` (tightened by CHAPTER 56 from an end-of-day cap, now that a real time exists), the same "never past the moment that matters" cap `payment_deadline`/`balance_deadline` already use. A pending request past its deadline is treated as a decline (`bookings.end_date` is never touched by expiry or by approval) and moves to `status = 'expired'` - a value the table's check constraint has allowed since CHAPTER 30, but nothing ever computed a deadline or set it until CHAPTER 46. Resolved two ways: a defensive check inline in `approve`/`reject`/`cancel` closes the narrow race window before the next cron tick, and `api/expire-booking-deadlines.ts` (already running every 15 minutes) is the authoritative path, notifying both sides.

### 8.5 Pickup no-show / non-return incidents (CHAPTER 31)

The fairness gap CHAPTER 27 left open: a renter forced to cancel because the car never showed up must **not** take the reliability hit, and a lister whose car is stolen or held past its return must be able to act. `api/booking-incident-action.ts` (service-role) handles three actions; all reuse the existing `getNoShowWindowState` / return-deadline gates (arrived + counterparty absent + past the 30-minute grace window).

- **`renter_no_car`** (renter, `/my-bookings`): the renter checked in, the lister did not, and the grace window passed. The booking is cancelled and **fully auto-refunded** through `processAutomaticRefundForBooking`; the `booking_cancellations` row is stamped `cancelled_by_role='lister'` so the renter's completion rate is untouched. **Cascade-aware fault:** if another `active` booking on the same car is already past its `end_date`, the overdue trip is flagged (`dispute_status='open'`) and blamed instead — the lister's cancellation is `strike_waived`.
- **`renter_no_show`** (lister, `/lister-bookings`): the lister checked in, the renter never arrived. The booking is cancelled; the renter is charged **a no-show fee counted in rental days (CHAPTER 91: `no_show_fee_days`, default 2, or `short_trip_no_show_fee_days`, default 0.75, for trips of 2 days or less, at the booking's average daily cost and never more than was paid; snapshotted per booking - a booking made before CHAPTER 91 keeps its `refund_late_renter_percent_snapshot` share)** and the rest is refunded. The renter's refund is queued as a `payments` `refund` row (`payment_method='manual_review'`) that a super-admin releases in Financial Reviews after confirming the return method. `booking_cancellations` is stamped `cancelled_by_role='renter'`, `was_late=true` — the lister's record is untouched.
- **`report_non_return`** (lister, `/lister-bookings`): an `active` trip is past its return time + grace and nobody has started completion. The lister picks a structured **reason** (CHAPTER 37 - renter unreachable, vehicle stolen/missing, accident/breakdown, or other), stored on `bookings.dispute_reason` alongside setting `bookings.dispute_status='open'` (a **sub-flag**, not a new booking status — it does not ripple through `status` filters). Opens a `booking_incident` support case (the reason is included in the ticket message too) and notifies the renter and admins. The booking is **not** cancelled; any refund stays on hold via the existing open-ticket gates.
...
- **Expiry** (`api/expire-booking-deadlines.ts`): a `downpayment_paid` booking past its `balance_deadline` is auto-cancelled. The financial consequence reuses the late-cancellation policy (since CHAPTER 91 a fee counted in rental days, `server/cancellationPolicy.ts`, snapshotted per booking at `create-booking` time) via the new `server/cancellationRefundPlan.ts` (`getCancellationRefundPlan`, `createManualRefundReview`, extracted so both the user-initiated `cancel` action in `api/booking-action.ts` and this cron share one calculation; `booking-action.ts` keeps its own untouched local copy for its critical payment path, importing nothing new). No new refund percentage was introduced. It is recorded in `booking_cancellations` with `cancelled_by_role='renter'`, so it counts against the renter's reliability the same way any other late cancellation does.
...
**An approved extension holds its days (CHAPTER 88).** From approval until it is paid or its `payment_deadline` passes, the days it adds (the day after the current return through the requested return) are taken for everyone: `create-booking` refuses them on that car and, for the same renter, on any car; the lister cannot block them; they show as booked on Car Details, in the Browse Cars date filter and on Vehicle Availability. A **pending** request holds nothing (first come, first served). When a lister accepts a booking that needs a pending request's days, the request is closed automatically (`rejected`, note "Closed automatically: those days now belong to another booking.") and both sides are told why - the lister sees the requests it will close before confirming. The rules live in `server/extensionHolds.ts` (request, approve, checkout, booking, accept) and are backstopped by `guard_booking_against_extension_holds`, `guard_extension_approval` and `guard_blackout_against_extension_holds`. If an extension is still paid but cannot be applied (e.g. document validity changed), the renter and lister are notified and emailed, and a refund ticket is opened.
...
- `src/lib/pwaInstall.ts` - `usePwaInstall()`, the source of truth for install availability (captures `beforeinstallprompt`, detects iOS Safari and already-standalone).
- `src/components/InstallButton.tsx` - the single install entry point: a small permanent button mounted in the landing page header (`LandingPage.tsx`) and `DashboardLayout.tsx`'s header, reachable from a first-time visitor's very first screen and every logged-in page afterward, always in the same place. An earlier dismissible one-time banner (`InstallPrompt.tsx`) was tried and removed - a renter who dismissed it had no other way to install later, and a second, permanent control made the dismissible one redundant rather than complementary. Went through two rounds of visibility fixes: (1) it originally hid itself whenever `beforeinstallprompt` hadn't fired - that event is one-shot-ish and Chrome throttles/suppresses re-firing it on repeated page loads within a session, so the button flickered in and out across refreshes; fixed by never gating on it. (2) a version at that point also stopped hiding on `isStandalone` (already installed), worried an uninstalled user would be stranded with no way back - over-corrected, since it then showed the button even while browsing from inside the already-running installed app, which is redundant. `isStandalone` is a live per-tab check of the CURRENT window, not a persistent "ever installed" flag - it already reads `false` again the moment the same site is opened in a normal browser tab (e.g. after later uninstalling), so hiding on it doesn't strand anyone; it only hides the button in the one context where showing it is actually confusing. Final behavior: hidden only when `isStandalone`; otherwise always visible regardless of `canInstall`/`showIosHint` state, and a click always does something useful - triggers the native prompt if available, shows the iOS "Add to Home Screen" steps on iOS Safari, or otherwise a generic "look for the install icon" fallback.
...
- **A07 Authentication Failures:** Supabase authentication, password recovery, OTP/MFA support, and admin route guards. Login throttling is layered: `src/lib/authLockout.ts` gives instant browser feedback, and DB Chapter 24's `password_verification_hook` (a Supabase Auth Hook) enforces the same progressive lockout server-side per account. Optional Cloudflare Turnstile captcha on the login/sign-up forms (`VITE_TURNSTILE_SITE_KEY` + Supabase Auth setting). Idle sign-out is 10 min for admins and 25 min for renters/listers; the Supabase session settings (session time-box + native inactivity timeout) are the server-side backstop and should be configured in the dashboard. Recovery, factor re-enrollment, and session evidence remain required.
...
| Super-admin | `/admin/announcements` | `AdminAnnouncementsPage`; send a message to the notification bell of every account, listers only, or renters only (`platform_announcements`, `send_platform_announcement`; CHAPTER 69). Bell only - no email, see below |
...
| Super-admin | `/admin/earnings` | `AdminEarningsPage`; what the platform earned from booking commission (ledger `4010`) and lister subscriptions (ledger `4030`), monthly chart, busiest month and weekday. Each total is counted twice - once from the ledger, once from the source records - and flags a mismatch instead of showing one number on trust. **Export CSV** writes the monthly summary (commission, subscriptions, total, plus a total line) for a chosen Manila date range, walking every row in the range rather than the capped page view, and records an `earnings_exported` audit entry |
| Super-admin | `/admin/financial-ledger` | `AdminFinancialLedgerPage`; journals and balanced entries. **Export CSV** writes every ledger line for a chosen Manila date range - date, event, booking, account code and name, debit and credit in pesos, memo, provider reference, status, and for a correction the record it reverses and why - one line per entry carrying its journal's context, paged in full so the file is never short, with a `money_records_exported` audit entry. SafeDrive does not issue BIR-accredited receipts; this is the source record a bookkeeper works from |
...
| `api/booking-early-return-action.ts` | POST; participant | Request (date+time) / approve / reject / cancel an early return; approve never touches `bookings.end_date`/`dropoff_time` (§8.4's missed-early-return fallback depends on this) and never refunds anything (CHAPTER 98 - no refund after handover) |
| `api/booking-incident-action.ts` | POST; participant | Pickup / return incidents: `renter_no_car` (renter checked in, no car → cancel + full refund, no reliability hit, cascade-aware fault), `renter_no_show` (lister checked in, renter absent → cancel, renter is charged a no-show fee counted in rental days - default 2 days, or 0.75 of a day for trips of 2 days or less - and the rest is refunded), `report_non_return` (active trip overdue → `bookings.dispute_status='open'` + a structured `dispute_reason`, support case, no cancel) |
...
| `api/account-deletion.ts` | GET/POST; signed-in user (super admin for `admin_delete`) | CHAPTER 96. GET: the caller's deletion status - grace days, scheduled date, what still blocks deleting. POST `schedule`: `schedule_account_deletion()` for the caller, emails the date and signs every session out. POST `cancel`: `cancel_account_deletion()` after the caller signed in and chose to keep the account. POST `admin_delete`: super admin runs `anonymize_user()` with a reason, emails the person at their old address and closes the login |
| `api/process-account-deletions.ts` | GET/POST; cron secret | Daily (CHAPTER 96): `run_due_account_deletions()` anonymizes accounts whose grace period ended (or puts them on legal hold if something opened), emails the closing notice, then closes the login (email replaced, sign-in banned, `profiles.login_closed_at`) of every deleted account not yet closed - including admin and privacy-request deletions |
| `api/flag-dormant-accounts.ts` | GET/POST; cron secret | Daily: auto-files a `deletion` `data_retention_requests` row (via `flag_dormant_accounts()`) for a regular-user account with no login activity past `platform_settings.dormant_account_days` (default 365) and no booking in progress - a super admin still has to review and execute it through Retention Requests, exactly like a user-submitted request |
...
| `api/mark-manual-refund.ts` | POST; super-admin | Record a super admin's manual refund decision: release as recommended, release a different amount, or deny (PHP 0). A changed amount needs a reason (10+ characters), is capped at what the booking collected and has not refunded, and is refused for cases that are not a judgement call (payment not applied, automatic refund failed, vehicle documents not cleared - `server/refundDecision.ts`). **The decided amount goes back through the original payment** (`releaseDecidedRefundToSource`, `server/refundAutomation.ts`): PayMongo refunds the payment the renter actually paid with, so no destination is chosen and no reference is typed. It reuses `buildRefundGroups`, so the automatic path's guards apply, updates the review's own refund row rather than creating a second one, and takes this road only when one original payment covers the whole decided amount. A pending provider refund stays pending - case open, no receipt, no journal, compensation waiting - until "Sync PayMongo Status". A manual GCash/Maya transfer is asked for only after SafeDrive answers that the provider cannot carry it, and a refund someone else released first is refused outright rather than offered one. Posts the final amount to the ledger, tells the renter (and, when changed, the lister) with the reason, and releases any lister compensation from the remainder |
...
| `api/send-license-decision-email.ts` | POST; admin/super-admin | Email a renter after their admin-recorded driver's-licence resubmission decision (approved/rejected, with reason) through the generic account-notification Resend template |
...
| `api/send-announcement-emails.ts` | POST; super-admin only | Email an announcement the super admin ticked "Also send by email" (CHAPTER 101), after `send_platform_announcement` has already delivered the bell notifications; safe to retry - per-recipient idempotency keys mean a run that times out can be sent again without emailing anyone twice, and `platform_announcements.emailed_at` / `email_sent_count` record the outcome |
...
| `src/lib/authLockout.ts` | `getAuthLockoutState`, `registerAuthFailure`, `clearAuthFailures`, `formatLockoutRemaining` | Browser-side progressive login lockout (instant UX feedback: 5 fails -> 5 min, 10 -> 10 min, ...; 24h reset). The authoritative version is the server-side hook (below) |
| `src/components/TurnstileWidget.tsx` | `TurnstileWidget`, `captchaConfigured` | Cloudflare Turnstile challenge on the login/sign-up forms. Inert unless `VITE_TURNSTILE_SITE_KEY` is set; when set, the token is passed to `signInWithPassword` / `signUp` / `signInWithOtp` / `resetPasswordForEmail`. Enable it together with captcha protection in the Supabase Auth settings |
| DB `public.password_verification_hook` (SQL Ch. 24) | Supabase "Password Verification Attempt" Auth Hook | Server-side, per-account progressive lockout mirroring `authLockout.ts` - enforced by Supabase on every password attempt, cannot be bypassed by clearing the browser, logs `lockout_started`. Fail-safe (any error -> allow). Activated by registering it in Auth -> Hooks; instant rollback = unregister |
...
| `src/lib/singleSession.ts` | `finalizeSingleSession`, `startSingleSessionGuard` | Single active session per account (SQL Ch. 57): the former revokes every other session's refresh token server-side (`supabase.auth.signOut({scope:"others"})`) once a login fully completes; the latter watches `profiles.active_session_token` (realtime + tab-focus + 45s poll) and force-signs-out a superseded tab promptly |
