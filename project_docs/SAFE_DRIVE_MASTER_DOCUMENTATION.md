...
- **`report_non_return`** (lister, `/lister-bookings`): an `active` trip is past its return time + grace and nobody has started completion. Sets `bookings.dispute_status='open'` (a **sub-flag**, not a new booking status — it does not ripple through `status` filters), opens a `booking_incident` support case, and notifies the renter and admins. The booking is **not** cancelled; the deposit and any refund stay on hold via the existing open-ticket gates.
