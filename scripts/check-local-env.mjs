...
if ((values.get("PAYMONGO_ENABLE_SANDBOX_PAYOUT_COMPLETION") || "").toLowerCase() === "true") {
  if (isConfigured("PAYMONGO_SECRET_KEY") && !secretKey.startsWith("sk_test_")) {
    ...
    console.warn(
      "[WARN] Demo money-movement mode enabled: payouts and cancellation refunds are recorded with full ledger + receipt but NO real PayMongo transfer (test key only). Unset this for a real launch.",
    );
  }
}
